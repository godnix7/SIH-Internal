...
class SOSAlert(Base):
    ...
    client_sos_id = Column(UUID(as_uuid=True), nullable=False, unique=True, index=True)
    ...
    incident_id = Column(UUID(as_uuid=True), index=True) # Can't FK to incident schema initially to avoid circular dep, or just use UUID
